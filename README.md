...
2. На бекенде сделать авто враппинг сокета в хендлеры, так чтобы потом просто добавлять новые классы хэндлеров не задумываясь о том, как их подключить к сокету.
3. Переделать схему rpc запросов. Не нравится мне, что тут ответ может быть одновременно и ошибкой, и верным объектом или примитивом любого типа. Лучше будет если схема будет, например такой:
...
4. Еще нужно ORM сюда затащить желательно. Я люблю objection.js.
5. Добавить возможность настраивать фронтенд через env
6. Добавить возможность настраивать nginx через env
7. Добавить возможность деплоить прод версию фронтенда и раздавать её через nginx

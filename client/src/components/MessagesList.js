    ...
    render() {
        const makeListItems = () => {
            const emptyList = (
                <Row>
                    <Alert variant="light">
                        {this.props.currentChatId ? 'Empty messages list' : 'Select chat for messaging'}
                    </Alert>
                </Row>
            )
                ...
                >
                    <Alert variant={item.isMy ? 'info' : 'success'}>
                        {item.author.isLoading ? (
                            ...
                            <>
                                <b>{item.isMy ? 'You' : item.author.login} </b>
                            ...
                            <Row>
                                <Col>
                                    <Form.Control
                                        ...
                                        disabled={!this.props.currentChatId}
                                    />
                                </Col>
                                <Col md={2}>
                                    <Button type="submit" disabled={!this.props.currentChatId}>
                                        Send
                                    </Button>
                                </Col>
                            </Row>
